...
model_path = r'models\detection\ysgyolo_1.2_OS1.0.pt'
print(f"\n=== 模型加载 ===")
...
ENABLE_FILTER = False                # 【已关闭】控制是否启用过滤标签 开=True 关=False
DEBUG_MODE = True                    # 【已开启】启用调试信息输出 开=True 关=False
...
def save_results(image_path, result, biaoqian_dir, output_image_dir, mask_folder_path, custom_color):
    ...
    if DEBUG_MODE:
        ...
        if result.boxes is None:
            print(f"    ❌ result.boxes 为 None - 未检测到任何对象")
            return
        ...
        for idx, box in enumerate(result.boxes):
            print(f"\n    原始数据 [{idx+1}]:")
            print(f"      box.cls: {box.cls}")
            print(f"      box.conf: {box.conf}")
            print(f"      box.xywh: {box.xywh}")
            print(f"      box.xywhn: {box.xywhn}")
            if hasattr(box, 'xyxy'):
                print(f"      box.xyxy: {box.xyxy}")
            if hasattr(box, 'xyxyn'):
                print(f"      box.xyxyn: {box.xyxyn}")
    ...
    with open(txt_path, write_mode) as f:
        for idx, box in enumerate(result.boxes):
            ...
            if DEBUG_MODE:
                print(f"    目标 {idx+1}: 类别={class_name}({cls}), 置信度={float(box.conf[0]):.3f}")
...
def process_image_folder(folder_path, current_folder_index, total_folders):
    try:
        ...
        for i in range(0, total_images, BATCH_SIZE):
            ...
            if DEBUG_MODE:
                print(f"\n{'='*60}")
                print(f"批次 {i//BATCH_SIZE + 1}/{(total_images-1)//BATCH_SIZE + 1}")
                print(f"推理参数: conf=0.5, iou=0.5, device={device}, imgsz=1024")
            ...
            for img_idx, (img_path, result) in enumerate(zip(batch_paths, results)):
                if DEBUG_MODE:
                    print(f"\n  === 结果 {img_idx+1} ===")
                    ...
                    if result.boxes is not None:
                        print(f"  result.boxes 类型: {type(result.boxes)}")
                        print(f"  result.boxes 长度: {len(result.boxes)}")
                        print(f"  result.boxes 内容: {result.boxes}")
                    if hasattr(result, 'orig_img'):
                        print(f"  原图尺寸: {result.orig_img.shape if result.orig_img is not None else 'None'}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        # 默认测试路径
        default_path = r"D:\xiazai\图片助手(ImageAssistant)_批量图片下载器\【第93話】…もう一度言ってくれ"
        if os.path.exists(default_path):
            print(f"使用默认路径: {default_path}")
            paths = [default_path]
        else:
            print("请将图片文件夹拖放到此脚本上运行。")
            print(f"或者修改脚本中的默认路径")
            input("按回车键退出...")
            sys.exit(0)
    else:
        paths = sys.argv[1:]
    ...
    print(f"\n共找到 {total_folders} 个文件夹：")
    for index, folder_path in enumerate(paths, start=1):
        print(f"{index}: {folder_path}")
    
    print(f"\n=== 运行参数 ===")
    print(f"调试模式: {'开启' if DEBUG_MODE else '关闭'}")
    print(f"类别筛选: {'开启' if ENABLE_FILTER else '关闭'}")
    if ENABLE_FILTER:
        print(f"  筛选类别: {FILTER_CLASSES}")
    print(f"置信度阈值: 0.5")
    print(f"IOU阈值: 0.5")
    print(f"生成掩膜: {'是' if GENERATE_MASK else '否'}")
    print(f"跳过已标注: {'是' if SKIP_BIAOQIAN else '否'}")
    print()
