...
### MangaJaNai 超分彩图检测
- **改进彩图检测算法，提高准确性**：
  - 使用 RGB 通道相关性作为主要判断依据（最可靠的方法）
  - 通道相关性 > 0.995 直接判定为黑白图，避免误判
  - 结合饱和度、标准差差异、相关性三个指标综合判断
  - 修复了低饱和度彩图被误判为黑白的问题（如饱和度 0.0661 的彩图）
  - 修复了 RGB 格式保存的黑白图被误判为彩图的问题（如相关性 1.0 的黑白漫画）
  - 经过 204 张图片测试验证，准确率 100%

### 调试日志增强
- **添加详细的调试日志以诊断类型错误**：
  - 在 `export_service.py` 中添加 lines 数据类型和值的详细日志
  - 在加载 JSON 时添加数据类型转换的调试信息
  - 在超分倍率缩放前后添加详细的类型和值检查日志
  - 帮助定位 `ufunc 'multiply' did not contain a loop` 错误的根本原因
