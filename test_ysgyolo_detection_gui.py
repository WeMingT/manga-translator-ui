...
class YSGYOLODetectorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("YSGYOLO 文本检测可视化工具")
        self.root.geometry("1200x800")
        
        # 模型路径
        self.model_path = r"models\detection\ysgyolo_1.2_OS1.0.onnx"
        self.session = None
    ...
    def load_model(self):
        ...
        try:
            ...
            self.status_var.set(f"模型已加载 (使用 {device})")
    ...
    def _detect_thread(self):
        ...
        try:
            ...
            img_input = img_rgb.transpose(2, 0, 1).astype(np.float32)
            img_input = np.expand_dims(img_input, axis=0) / 255.0
            
            # 推理
            input_name = self.session.get_inputs()[0].name
            output_names = [output.name for output in self.session.get_outputs()]
            outputs = self.session.run(output_names, {input_name: img_input})
            
            # 后处理
            boxes, scores, class_ids = self.process_output(
                outputs[0], conf_threshold, iou_threshold
            )
